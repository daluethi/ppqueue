...
@implementation PPQueueStorageEngine
...
- (id)init
{
    ...
    if (self) {
        ...
        NSString *path                  = [documentsDirectory stringByAppendingPathComponent:@"ppqueue_0.5.0d.db"];
        ...
        [self.queue inDatabase:^(FMDatabase *db) {
            [db executeUpdate:@"CREATE TABLE IF NOT EXISTS queue ("
                    ...
                    "udef_1 TEXT, "
                    "udef_2 TEXT)"];
            [db executeUpdate:@"CREATE INDEX IF NOT EXISTS queue_index on queue (priority ASC, id ASC, task ASC)"];
...
- (void)createJob:(id)data forTask:(id)task withPriority:(NSUInteger)priority {
    ...
    [self.queue inDatabase:^(FMDatabase *db) {
        [db executeUpdate:@"INSERT INTO queue (task, data, priority) VALUES (?, ?, ?)", task, dataString, @(priority)];
        [self _databaseHadError:[db hadError] fromDatabase:db];
    }];
}
...
- (void)removeOldJobs:(NSString*)timeInterval {
    [self.queue inDatabase:^(FMDatabase *db) {
        [db executeUpdate:@"DELETE FROM queue WHERE CAST(stamp AS INTEGER) < CAST(strftime('%s', 'now', ?) as integer)", timeInterval];
        [self _databaseHadError:[db hadError] fromDatabase:db];
    }];
}
...
- (NSDictionary *)fetchJob
{
    ...
    [self.queue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:@"SELECT * FROM queue ORDER BY priority ASC, id ASC LIMIT 1"];
        [self _databaseHadError:[db hadError] fromDatabase:db];
...
- (NSDictionary *)fetchJobForTask:(id)task
{
    ...
    [self.queue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:@"SELECT * FROM queue WHERE task = ? ORDER BY priority ASC, id ASC LIMIT 1", task];
        [self _databaseHadError:[db hadError] fromDatabase:db];
...
- (NSDictionary *)_jobFromResultSet:(FMResultSet *)rs
{
    NSDictionary *job = @{
            ...
            @"priority":    [NSNumber numberWithInt:[rs intForColumn:@"priority"]],
